...
for f in "$HERE"/repros/*.bend; do
  ...
  if grep -q '^def main() -> IO' "$f"; then
    ...
    echo "  C:    $("$OUT/$n" x 2>&1 | head -c 300 | od -An -c | tr -s ' ' | tr '\n' ' ' | cut -c1-200)"
    echo "  bun:  $(bun "$OUT/$n.js" x 2>&1 | grep -v '^ ' | head -3 | tr '\n' '|' | cut -c1-200)"
    echo "  node: $(node "$OUT/$n.js" x 2>&1 | grep -E 'Maximum|^[0-9A-Za-z-]' | head -3 | tr '\n' '|' | cut -c1-200)"
  fi
